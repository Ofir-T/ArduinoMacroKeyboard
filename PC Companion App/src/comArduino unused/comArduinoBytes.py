...
def sendToArduino(sendStr):
  ...
  ser.write(bytes(adjSendStr, 'ascii'))
...
def encodeHighBytes(inBytes):
  global specialByte
  
  outBytes = bytearray()
  s = len(inBytes)
  
  for n in range(s): # for byte in inBytes:?
    x = inBytes[n]
    
    if x >= specialByte:
       outBytes += specialByte
       outBytes += x - specialByte
    else:
       outBytes += x
       
  print("encINSTR  " + bytesToString(inBytes))
  print("encOUTSTR " + bytesToString(outBytes))
...
def bytesToString(data): # in hex format

  byteString = ""
  n = len(data)
  
  for s in range(n): # for s in data:? # can just do byteString.hex('-') or list of int.from_bytes(s)
    byteString += data[s].hex()
    byteString += "-"
...
def displayDebug(debugBytes):

   n = len(debugBytes) - 3
   debugStr = debugStr.hex(' ')
...
def waitForArduino():
   ...
   # it also ensures that any bytes left over from a previous message are discarded
    ...
    msg = bytearray()
    while msg.find(b'Arduino Ready') == -1:
...
byte = lambda string: [ ord(char) for char in string if (ord(char) < 128)]# else ]
string = lambda bytestr: [ chr(byte) if (byte < 128) else 'z' for byte in bytestr]#str(int(byte)) for byte in bytestr]
...
testData = []
testData.append('gb')
# testData.append('sbkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkh')
testData.append(b'\x73\x62\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00'.decode('ascii'))
testData.append(b'\x73\x62\x6b\x68\x00\x73\x73\x74\x72\x69\x6e\x67\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00\x6b\x68\x00'.decode('ascii'))
testData.append('gb')
#testData.append('ge')
