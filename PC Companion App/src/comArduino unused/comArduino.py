...
def sendToArduino(sendStr):
  ...
  ser.write(bytes(adjSendStr, "utf-8"))
...
def decodeHighBytes(inStr):
  ...
  while n < len(inStr):
     if inStr[n] == specialByte:
        n += 1
        x = specialByte + inStr[n]
     else:
        x = inStr[n]
     outStr.append(x)
     n += 1
...
def bytesToString(data):
  ...
  for s in range(0, n):
    byteString = byteString + str(data[s])
...
def waitForArduino():
   ...
   # it also ensures that any bytes left over from a previous message are discarded
    ...
    # while(True):
    #     print(ser.read())
    while (msg.decode("utf-8").find("Arduino Ready") == -1):
...
print(chr(startMarker))
print(chr(endMarker))
print(chr(specialByte))
...
testData.append("{8sbkh␀kh␀kh␀kh␀kh␀kh␀kh␀kh␀kh␀kh␀kh␀kh␀kh␀kh␀kh␀kh␀kh␀kh␀}")
...
while n < numLoops:
  ...
  if ser.inWaiting() > 0:
    ...
    if ord(dataRecvd[0]) > 0:
      displayData(dataRecvd[1].decode())
