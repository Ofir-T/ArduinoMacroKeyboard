...
#region keycodes
    ...
    def get_name_of(cls, code):
        ...
        for k, v in cls.keyboard_keycode.items():
            if (v == code):
                return k
...
class Chr(Command):
    ...
    def to_bytes():
        return b'k' + self.code.to_bytes(1, sys.byteorder) + b'\x00'
...
class Cons(Command):
    ...
    def to_bytes():
        return b'c' + self.code.to_bytes(1, sys.byteorder) + b'\x00'
...
class CStr(Command):
    ...
    def to_bytes():
        return bytes(self.cStr) + b'\x00' #[ ord(char) for char in string if (ord(char) < 128)]

def command_from_str(string):
    if (string[0]=='k'):
        new_comm = Chr(string[1])
    elif(string[0]=='c'):
        new_comm = Cons(string[1])
    elif(string[0]=='s'):
        new_comm = CStr(string[1:])
    # else
    #     return 0
    return new_comm # strlen((char*)data) + int(dest->nullTerm())
...
#region keycodes
    ...
    def get_code_of(cls, name) -> str: #without the prefix
        ...
        #     if (name == key.split("_",1)[1]):
        #         return value
        if name in cls.keyboard_keycode:
            return cls.keyboard_keycode[name]
        elif name in cls.consumer_keycode:
            return cls.consumer_keycode[name]
        return None
    ...
    @classmethod
    def get_type_of(cls, name) -> str:
        """Returns the type of the named command."""
        if name in cls.keyboard_keycode.items():
            return 'k' #if (name == key.split("_",1)[1]):
        elif name in cls.consumer_keycode.items():
                return 'c'
        elif name == '""':
                return 's'
        return None
    ...
    @classmethod
    def get_bind_of(cls, name) -> str:
        """Returns a command string."""
        if name in cls.keyboard_keycode:
            return 'k' + chr(cls.keyboard_keycode[name]) #if (name == key.split("_",1)[1]):
        elif name in cls.consumer_keycode:
