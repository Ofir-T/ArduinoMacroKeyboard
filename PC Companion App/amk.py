"""
this module contains the <keyname>:<keycode> pairs used in the keyboard,
and defines the keyboard object used to store all the keyboard data.
"""
...
#region constructors
    ...
    def init_from_dict(cls, dict, callback):
        ...
        name = dict['name']
        new_keyboard = cls(rows, cols, sets, bindings, orientaion, active_set, name, callback)
        return new_keyboard
#endregion
...
#region getters
    ...
    def is_full(self):
        uninitialized_attributes = [k for k, v in vars(self).items() if (not k.startswith('_')) and v==None]
...
#region utility functions
    ...
    def to_dict(self):
        attributes_dict = {key:value for key, value in self.__dict__.items() if not key.startswith('__') and not callable(key)} # [k for k, v in dict(self) if not k.startswith('_')]
        # print(attributes_dict)
        return attributes_dict

    def save_to_arduino(self): #only sends bindings for now
...
#region keycodes
    ...
    def get_name_of(cls, code):
        """ gets the key name associated with the key code """
...
if __name__ == "__main__":
    ...
    example_keypad_dict = {'num_rows': 3, 'num_cols': 3, 'num_sets': 2, 'bindings': [[104, 105, 106, 107, 108, 109, 110, 111, 112], [30, 20, 9, 7, 33, 26, 5, 8, 21]], 'orientation': 3, 'active_set': 0}
