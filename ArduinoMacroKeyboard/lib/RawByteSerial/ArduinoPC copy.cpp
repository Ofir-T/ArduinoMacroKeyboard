...
byte line[MAX_MSG_LENGTH+2]; // +2 for start/end markers
...
void getSerialData2() 
{
  ...
  {
    bytesRecvd = 0;
    byte x = 'z'; // anything except start/end marker
    while(Serial.available() > 0 && x != endMarker) //&& bytesRecvd <= sizeof(line)) 
    {
      x = Serial.read();
      line[bytesRecvd] = x;
      bytesRecvd ++;
